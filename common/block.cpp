...
int encrypt_key_val(uint8_t *enc_key, uint8_t *bytesOut, uint8_t *iv, uint8_t *tag, uint32_t data_key, uint8_t *data,
...
int encrypt_read_key(uint8_t *enc_key, uint8_t *bytesOut, uint8_t *iv, uint8_t *tag, uint32_t data_key,
  uint32_t *replay_ctr, bool immediate_inc) {
  int rv;
  uint8_t *buf;
  int sz = get_key_val_buf_sz();
...
int decrypt_key_val(uint8_t *dec_key, uint32_t *data_key, uint8_t *data, const uint8_t *bytesIn, const uint8_t *iv,
  uint8_t *tag, uint32_t *replay_ctr, bool immediate_inc) {
  int rv;
  uint8_t *buf;
  int sz = get_key_val_buf_sz();
...
int encrypt_key_val_pairs(uint8_t *enc_key, uint8_t *bytesOut, uint8_t *iv, uint8_t *tag, uint32_t *data_key_arr,
  uint8_t **data_arr, int num_pairs, uint32_t *replay_ctr, bool immediate_inc) {
  int rv;
  uint8_t *buf;
...
int decrypt_key_val_pairs(uint8_t *dec_key, uint32_t *data_key_arr, uint8_t **data_arr, int num_pairs,
  const uint8_t *bytesIn, const uint8_t *iv, uint8_t *tag, uint32_t *replay_ctr, bool immediate_inc) {
  int rv;
  uint8_t *buf;
  int total_len = get_key_val_buf_sz(num_pairs);
  int pair_len = sizeof(uint32_t) + BLOCK_LEN;
...
AssignedRequest::AssignedRequest(KeyBlockPair &req_in, SubOramID SID_in, uint8_t tag_in)
    : req(req_in), SID(SID_in), tag(tag_in) {}
};  // namespace lb_types
