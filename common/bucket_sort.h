...
std::thread spawn_enclave_thread(EnclaveFn &&enc_fn, Args &&...args) {
  std::thread enclave_thread(
    [](typename std::decay<EnclaveFn>::type &&enc_fn, typename std::decay<Args>::type &&...args) {
...
class EnclaveBucketSorter {
 ...
 public:
  EnclaveBucketSorter() {}
  EnclaveBucketSorter(shared_sort_state<T> *s, uint8_t *aes_key, uint8_t *iv, int n_threads = 1)
      : s(s), aes_key(aes_key), iv(iv), n_threads(n_threads) {}
  ...
  int sort(std::vector<T> &blocks, Comparator cmp, int thread_idx = 0) {
    ...
    unsigned char *key;
    printf("levels: %d, z: %d, buckets: %d, blocksize: %zu\n", params.log_total_buckets, params.z, params.total_buckets,
      sizeof(T));
    uint8_t *tmp;
    bs::state.tmp.clear();
    for (int i = 0; i < n_threads; i++) {
    ...
    CHECK_A(key = (unsigned char *)malloc(16 * sizeof(unsigned char)));
    {
      ...
      for (int j = 0; j < params.total_buckets / BUF_BUCKETS; j++) {
          fetch_from_queue();
          for (int k = 0; k < BUF_BUCKETS / 2; k++) {
              merge_split(&buf[k].blocks[0], &buf[k].blocks[params.z], buf[k].blocks, tags, params.z,
      params.log_total_buckets, 0, params.log_total_buckets); for (int i = 0; i < 2*params.z; i++) { tags[i] = (bool)
      (buf[k].blocks[i].bucket_sort_pos & params.total_buckets); tags[i] = 1 - tags[i];
              }
 ...
 private:
  ...
  void merge_split(T *input0, T *input1, std::vector<T> &input, uint8_t *tags, int z, int i, int idx, int bitlen) {
      ...
      for (int j = 0; j < 2; j++) {
        ...
        for (int t = 0; t < TAG_LEN; t++) {
          if (expected_mac[t] != buf_ct[2 * i + j].tag[t]) {
            printf("[t%d] [i %d, j %d] [idx %d] [strt %d] TAG DOES NOT MATCH:\n\texpected 0x", thread_id, i, j,
              local_bucket_idx, bucket_idx);
            for (int t = 0; t < TAG_LEN; t++) {
  ...
  void merge_split_curr_buffer(int thread_id) {
    ...
    for (int k = bounds.first; k < bounds.second; k++) {
      merge_split(&buf[k].items[0], &buf[k].items[params.z], buf[k].items, tags, params.z, bs::state.i, 0,
  ...
  void process_last_layer_buffer(int thread_id) {
    ...
    for (int k = bounds.first; k < bounds.second; k++) {
      ...
      merge_split(&buf[k].items[0], &buf[k].items[params.z], buf[k].items, tags, params.z, params.log_total_buckets, 0,
        params.log_total_buckets);
      ...
      ObliviousSort(
        buf[k].items.begin() + params.z, buf[k].items.begin() + params.z + bucket_end, cmp_bucket_item_pos<T>);
  ...
  void mock_enclave_sort(std::vector<T> &blocks, Comparator cmp, int n_threads) {
    ...
    for (int i = 0; i < n_threads; i++) {
      threads.push_back(std::thread(
        [cmp](std::vector<T> &blocks, EnclaveBucketSorter<T> &enc_sorter, int thread_id) {
