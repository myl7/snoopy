...
oe_result_t generate_certificate_and_pkey(X509 *&certificate, EVP_PKEY *&pkey) {
  ...
  result =
    generate_key_pair(&public_key_buffer, &public_key_buffer_size, &private_key_buffer, &private_key_buffer_size);
  ...
  result = oe_generate_attestation_certificate(certificate_subject_name, private_key_buffer, private_key_buffer_size,
    public_key_buffer, public_key_buffer_size, &output_certificate, &output_certificate_size);
...
oe_result_t load_tls_certificates_and_keys(SSL_CTX *ctx, X509 *&certificate, EVP_PKEY *&pkey) {
  oe_result_t result = OE_FAILURE;
...
int read_from_session_peer(SSL *&ssl_session, const char *payload, size_t payload_length) {
  int ret = -1;
  ...
  do {
    ...
    if ((bytes_read != payload_length) || (memcmp(payload, buffer, bytes_read) != 0)) {
      printf(
...
int write_to_session_peer(SSL *&ssl_session, const char *payload, size_t payload_length) {
  int bytes_written = 0;
  int ret = 0;

  while ((bytes_written = SSL_write(ssl_session, payload, payload_length)) <= 0) {
