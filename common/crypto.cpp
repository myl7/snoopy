...
int symm_encrypt(
  EVP_CIPHER_CTX *ctx, uint8_t *bytesOut, uint8_t *iv, uint8_t *tag, const uint8_t *bytesIn, uint8_t *curr_iv) {
  inc_iv(curr_iv);
...
int symm_decrypt(EVP_CIPHER_CTX *ctx, uint8_t *bytesOut, const uint8_t *iv, uint8_t *tag, const uint8_t *bytesIn) {
  int rv = ERROR;
  int bytesFilled = 0;
  // return OKAY;
...
int symm_decrypt(
  const uint8_t *key, uint8_t *bytesOut, const uint8_t *iv, uint8_t *tag, const uint8_t *bytesIn, int len) {
  int rv = ERROR;
  int bytesFilled = 0;
  int ct_len = 0;
  EVP_CIPHER_CTX *ctx;
...
int encrypt_key_val(EVP_CIPHER_CTX *ctx, uint8_t **write_ptr, uint32_t key, uint8_t *data) {}

int symm_encrypt(EVP_CIPHER_CTX *ctx, uint8_t *bytesOut, uint8_t *iv, uint8_t *tag, uint8_t *curr_iv,
...
int symm_decrypt(
  EVP_CIPHER_CTX *ctx, uint8_t *bytesOut, const uint8_t *iv, uint8_t *tag, const uint8_t *bytesIn, int len) {
  int rv = ERROR;
  int bytesFilled = 0;
  CHECK_C(EVP_DecryptInit_ex(ctx, NULL, NULL, NULL, iv));
  CHECK_C(EVP_DecryptUpdate(ctx, bytesOut, &bytesFilled, bytesIn, len));
