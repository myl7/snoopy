 ...
 * name in the list.
 */
static const oe_claim_t *find_claim(const oe_claim_t *claims, size_t claims_size, const char *name) {
...
static bool verify_signer_id(
  const char *pem_key_buffer, size_t pem_key_buffer_len, uint8_t *expected_signer, size_t expected_signer_size) {
  ...
  if (oe_sgx_get_signer_id_from_public_key(
        pem_key_buffer, pem_key_buffer_len, calculated_signer, &calculated_signer_size) != OE_OK) {
  ...
  if (memcmp(calculated_signer, expected_signer, expected_signer_size) != 0) {
    ...
    for (size_t i = 0; i < expected_signer_size; i++) {
      printf("0x%x - 0x%x\n", (uint8_t)expected_signer[i], (uint8_t)calculated_signer[i]);
    }
    return false;
  }
...
// inside this routine.
oe_result_t enclave_claims_verifier(oe_claim_t *claims, size_t claims_length, void *arg, const char pubkey[]) {
  oe_result_t result = OE_VERIFY_FAILED;
  const oe_claim_t *claim;

  (void)arg;
  printf(TLS_CLIENT
    "enclave_claims_verifier is called with the claims from "
    "the evidence:\n");
  ...
  if (claim->value_size != sizeof(uint32_t)) {
    printf("security_version size(%lu) checking failed\n", claim->value_size);
    goto done;
  }
  printf(TLS_SERVER "\nsecurity_version = %d\n", *claim->value);
  ...
  if (claim->value_size != OE_UNIQUE_ID_SIZE) {
    printf(TLS_CLIENT "unique_id size(%lu) checking failed\n", claim->value_size);
    goto done;
  }
  ...
  if (claim->value_size != OE_PRODUCT_ID_SIZE) {
    printf(TLS_CLIENT "product_id size(%lu) checking failed\n", claim->value_size);
    goto done;
  }
  printf(TLS_CLIENT "\nproduct_id :\n");
  ...
  if (claim->value_size != OE_SIGNER_ID_SIZE) {
    printf(TLS_CLIENT "signer_id size(%lu) checking failed\n", claim->value_size);
    goto done;
  }
  printf(TLS_CLIENT "\nverify signer_id:\n");
  for (size_t i = 0; i < claim->value_size; i++) printf("0x%0x ", (uint8_t)claim->value[i]);
  ...
  printf(TLS_CLIENT "\n** sizeof pubkey = %zu, sizeof SERVER_PUBKEY = %zu\n", strlen(pubkey), sizeof(SERVER_PUBKEY));
...
oe_result_t enclave_claims_verifier_server(oe_claim_t *claims, size_t claims_length, void *arg) {
  return enclave_claims_verifier(claims, claims_length, arg, SERVER_PUBKEY);
}

oe_result_t enclave_claims_verifier_client(oe_claim_t *claims, size_t claims_length, void *arg) {
  return enclave_claims_verifier(claims, claims_length, arg, CLIENT_PUBKEY);
}
...
int verify_callback(int preverify_ok, X509_STORE_CTX *ctx, bool isServer) {
  int ret = 0;
  int der_len = 0;
  ...
  printf(TLS_CLIENT "verify_callback called with preverify_ok=%d\n", preverify_ok);
  ...
  printf(TLS_CLIENT "der=%p buff=%p buff moved by %d offset der_len=%d\n", der, buff, (int)(buff - der), der_len);

  printf(" verifying certificate start \n");
  // verify tls certificate
  oe_verifier_initialize();
  if (isServer) {
    result = oe_verify_attestation_certificate_with_evidence(der, der_len, enclave_claims_verifier_server, nullptr);
  } else {
    result = oe_verify_attestation_certificate_with_evidence(der, der_len, enclave_claims_verifier_client, nullptr);
  }
  if (result != OE_OK) {
  ...
  if (err != X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT) {
    printf(TLS_CLIENT "verifying SGX certificate extensions ... %s\n", ret ? "succeeded" : "failed");
  }
...
int verify_callback_server(int preverify_ok, X509_STORE_CTX *ctx) {
  return verify_callback(preverify_ok, ctx, true);
}

int verify_callback_client(int preverify_ok, X509_STORE_CTX *ctx) {
  return verify_callback(preverify_ok, ctx, false);
}
