...
oe_result_t generate_key_pair(
  uint8_t **public_key, size_t *public_key_size, uint8_t **private_key, size_t *private_key_size) {
  ...
  result = oe_get_private_key_by_policy(OE_SEAL_POLICY_UNIQUE, &params, private_key, private_key_size, NULL, NULL);
  if (result != OE_OK) {
...
bool verify_signer_id(const char *siging_public_key_buf, size_t siging_public_key_buf_size, uint8_t *signer_id_buf,
  ...
  if (oe_sgx_get_signer_id_from_public_key(siging_public_key_buf, siging_public_key_buf_size, signer, &signer_size) !=
      OE_OK) {
