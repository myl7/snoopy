...
oe_result_t generate_certificate_and_pkey(mbedtls_x509_crt *certificate, mbedtls_pk_context *private_key) {
  ...
  result = oe_get_attestation_certificate_with_evidence(&_uuid_sgx_ecdsa, certificate_subject_name, private_key_buffer,
    private_key_buffer_size, public_key_buffer, public_key_buffer_size, &output_certificate, &output_certificate_size);
  if (result != OE_OK) {
    printf("oe_generate_attestation_certificate failed with %s\n", oe_result_str(result));
    goto exit;
  }

  // create mbedtls_x509_crt from output_cert
  ret = mbedtls_x509_crt_parse_der(certificate, output_certificate, output_certificate_size);
  if (ret != 0) {
  ...
  ret = mbedtls_pk_parse_key(private_key, (const unsigned char *)private_key_buffer, private_key_buffer_size, NULL, 0);
  if (ret != 0) {
