...
class SuboramServiceImpl final : public Oram::Service {
 public:
  ...
  Status BatchedAccessKeys(ServerContext *context, const OramRequests *req, OramResponses *resp) override {
    ...
    cout << "process batch [" << num_reqs
         << "] total: " << std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() << endl;
...
int main(int argc, const char *argv[]) {
  ...
  SuboramHost host(config[NUM_BLOCKS], config[NUM_BALANCERS], config[NUM_SUBORAMS], config[SUBORAM_ID],
    config[PROTOCOL_TYPE], config[SORT_TYPE], config[MODE], config[THREADS],
    config[OBLIX_BASELINE][OBLIX_BASELINE_160]);
...
exit:
  ...
  if (enclave) host.terminate_enclave();
...
oe_enclave_t *SuboramHost::create_enclave(const char *enclave_path) {
  ...
  oe_result_t result =
    oe_create_suboram_enclave(enclave_path, OE_ENCLAVE_TYPE_SGX, OE_ENCLAVE_FLAG_DEBUG, NULL, 0, &enclave);

  if (result != OE_OK) {
    printf("Host: oe_create_remoteattestation_enclave failed. %s", oe_result_str(result));
  } else {
...
void SuboramHost::init_enclave() {
  ...
  if (protocol == OUR_PROTOCOL) {
    set_params(enclave, &ret, num_blocks, blocks_per_bucket, num_balancers, num_suborams, suboram_id, &num_local_blocks,
      num_threads);
    if (mode == BENCH_SORT) {
...
void SuboramHost::mock_init_enclave() {
  ...
  if (mode == BENCH_PROCESS_BATCH) {
    ...
    std::thread enclave_thread([&, this] { mock_suboram.init(&s); });
    enclave_thread.join();
    host_thread.join();
  } else {
...
void SuboramHost::run_process_batch(uint8_t *in_ct, uint8_t *in_iv, uint8_t *in_tag, uint8_t *out_ct, uint8_t *out_iv,
  uint8_t *out_tag, int batch_sz, uint32_t balancer_id) {
  if (protocol == OUR_PROTOCOL) {
    std::thread host_thread([this] { fill_block_ct(); });
    std::vector<std::thread> threads;
    for (int i = 0; i < num_threads; i++) {
      threads.push_back(spawn_enclave_thread_on_cpu(i, table_process_batch_parallel, enclave, &ret, in_ct, in_iv,
        in_tag, out_ct, out_iv, out_tag, batch_sz, balancer_id, i));
    }
    ...
    std::thread enclave_thread = spawn_enclave_thread(
      oblix_process_batch, enclave, &ret, in_ct, in_iv, in_tag, out_ct, out_iv, out_tag, batch_sz, balancer_id);
    enclave_thread.join();
    std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
...
void SuboramHost::run_mock_process_batch(uint8_t *in_ct, uint8_t *in_iv, uint8_t *in_tag, uint8_t *out_ct,
  uint8_t *out_iv, uint8_t *out_tag, int batch_sz, uint32_t balancer_id) {
  if (protocol == OUR_PROTOCOL) {
    std::thread host_thread([this] { fill_block_ct(); });
    std::vector<std::thread> threads;
    for (int i = 0; i < num_threads; i++) {
      threads.push_back(std::thread([&, this, i] {
        mock_suboram.ecall_process_requests_parallel(
          &ret, in_ct, in_iv, in_tag, out_ct, out_iv, out_tag, batch_sz, balancer_id, i);
      }));
    }
    for (auto &thread : threads) {
      thread.join();
...
void SuboramHost::bench_scan_blocks(int num_trials) {
  ...
  printf("Scan time for %d total blocks: %f us\n", num_blocks,
    ((double)std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()) / ((double)num_trials));
}
...
((double)std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count())); printf("Finished processing
batch\n");
}*/
