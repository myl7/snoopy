...
class SuboramHost {
 ...
 public:
  ...
  SuboramHost(int num_blocks, int num_balancers, int num_suborams, int suboram_id, std::string protocol,
    std::string sort_type, std::string mode, int num_threads, json oblix_baseline)
      : ret(OKAY),
  ...
  void run_shuffle();
  void run_process_batch(uint8_t *in_ct, uint8_t *in_iv, uint8_t *in_tag, uint8_t *out_ct, uint8_t *out_iv,
    uint8_t *out_tag, int batch_sz, uint32_t balancer_id);
  void run_mock_process_batch(uint8_t *in_ct, uint8_t *in_iv, uint8_t *in_tag, uint8_t *out_ct, uint8_t *out_iv,
    uint8_t *out_tag, int batch_sz, uint32_t balancer_id);
