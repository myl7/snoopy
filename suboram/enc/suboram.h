...
class SuboramDispatcher {
 ...
 public:
  ...
  int gen_request_table(
    std::vector<req_table_key_bucket_item> &table, std::vector<uint32_t> &idxs, uint32_t *key_arr, uint32_t batch_sz);
  int assign_new_permutation();
  int update_block_map();
  int process_requests_parallel(
    uint32_t batch_sz, uint32_t num_table_buckets, uint32_t *key_arr, uint8_t **in_data_arr, uint8_t **out_data_arr);

  void ecall_process_requests_parallel(int *ret, uint8_t *in_ct, uint8_t *in_iv, uint8_t *in_tag, uint8_t *out_ct,
